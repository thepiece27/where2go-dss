...
function showDetail(poi) {
  ...
  panel.replaceChildren(
    ...
    node("p",profile.typical_minutes?`Thời lượng: ${profile.short_minutes}/${profile.typical_minutes}/${profile.long_minutes} phút · ${profile.method}`:"Chưa có hồ sơ thời lượng"),
    actions,
  );
...
async function loadPois() {
  ...
  try {
    ...
    for(const poi of data.pois) {
      ...
      if(markerLayer&&Number.isFinite(poi.latitude)&&Number.isFinite(poi.longitude)) L.circleMarker([poi.latitude,poi.longitude],{radius:5,color:"#0f766e"}).addTo(markerLayer).on("click",()=>showDetail(poi)).bindTooltip(node("span",poi.name));
    }
    if(data.total>data.pois.length)list.append(node("p",`Đang hiển thị ${data.pois.length} điểm. Hãy lọc để xem cụ thể hơn.`));
    if(data.pois.length)showDetail(data.pois.find(poi=>poi.poi_id===state.selectedId)||data.pois[0]);
    else{$("detailPanel").replaceChildren(node("p","Không có địa điểm phù hợp."));state.selectedId=null;}
...
function renderPlan(data) {
  ...
  for(const block of data.blocks||[]) {
    ...
    if(block.role==="attraction"){
      attractionIndex++;title.onclick=()=>{const poi=state.pois.find(item=>item.poi_id===block.poi_id);if(poi)showDetail(poi);};
    } else title.disabled=true;
    card.append(title,node("p",`${block.start_time}–${block.end_time} · ${block.duration_minutes} phút`));
    if(block.role==="attraction") {
      card.append(node("p",`Giờ: ${block.hours_status==="known"?"đã có dữ liệu":"cần kiểm tra"} · Tiếp cận: ${block.access_status}`));
  ...
  if(data.ranking){const details=node("details");details.append(node("summary","Vì sao có kết quả này?"));details.append(node("p",`Fuzzy AHP + TOPSIS · CR=${data.ranking.cr.toFixed(3)}`));details.append(node("p","Trọng số: "+Object.entries(data.ranking.weights).map(([key,value])=>`${key} ${value.toFixed(3)}`).join(" · ")));panel.append(details);}
...
$("locationFilter").onchange=()=>{const centers={"Hà Nội":[21.0285,105.8542],"Đà Nẵng":[16.0544,108.2022]};const center=centers[$("locationFilter").value];if(center){$("latitude").value=center[0];$("longitude").value=center[1];if(map)map.setView(center,12);}updateStart();invalidatePlan();loadPois();};
