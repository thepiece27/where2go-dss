...
const map = window.L
  ? L.map("map", { preferCanvas: true, minZoom: 4 }).setView([16, 108], 5)
  : null;
let markerLayer,
  startMarker,
  routeLayer,
...
function basemapStyle(feature) {
  const kind = feature.properties?.kind,
    detail = feature.properties?.class;
  if (kind === "focus_boundary")
    return {
      color: "#aab6bb",
      weight: 1,
      fillColor: "#f1f4ec",
  ...
  if (kind === "green")
    return { stroke: false, fillColor: "#c9dfbc", fillOpacity: 0.8 };
  if (kind === "water")
    return { stroke: false, fillColor: "#a5d3e3", fillOpacity: 1 };
  if (kind === "waterway" || kind === "coastline")
    return { color: "#73b3cd", weight: 1.5 };
  if (kind === "admin")
    return { color: "#a1a5b1", weight: 1, dashArray: "4 5", fill: false };
  if (kind === "road")
    return {
      color:
        detail === "major"
          ...
          : detail === "secondary"
            ? "#ead1a5"
            : "#fff",
      weight: detail === "major" ? 3 : 1.5,
    };
  return { color: "#abb6b8", weight: 1, fillOpacity: 0.2 };
}
function clearBasemap() {
  for (const layer of [basemapLayer, basemapLocalLayer])
...
function setOffline(value) {
  ...
  $("mapStatus").textContent = value
    ? "Đang dùng bản đồ dự phòng. Chi tiết offline có tại Hà Nội và Đà Nẵng."
    : "";
...
async function loadBasemap(location) {
  ...
  try {
    ...
    if (!data) {
      data = await api(`/api/v2/basemaps/${config.slug}`);
      localCache.set(config.slug, data);
    }
    ...
    basemapLayer = L.geoJSON(data, {
      ...options,
      filter: (f) =>
        !(f.properties?.kind === "road" && f.properties?.class === "local"),
    }).addTo(map);
    basemapLocalLayer = L.geoJSON(data, {
      ...options,
      filter: (f) =>
        f.properties?.kind === "road" && f.properties?.class === "local",
    });
    ...
    if (version === state.basemapVersion)
      $("mapStatus").textContent =
        "Bản đồ dự phòng toàn quốc; chi tiết địa phương chưa sẵn sàng.";
  }
}
function markerColor(category) {
  if (["beach", "nature_area", "park", "viewpoint"].includes(category))
    return "#137c8b";
  if (foodCategories.has(category)) return "#b56522";
  if (["temple", "historic", "museum", "old_quarter"].includes(category))
    return "#7552a0";
  return "#2368aa";
}
function highlightPoi(poi) {
  ...
  selectedMarker = L.circleMarker([poi.latitude, poi.longitude], {
    radius: 11,
    color: "#d64c28",
    weight: 3,
    fillOpacity: 0.12,
...
function renderMapMarkers() {
  ...
  for (const feature of mapFeatures) {
    const [lon, lat] = feature.geometry.coordinates,
      point = map.project([lat, lon], zoom);
  ...
  for (const group of buckets.values()) {
    if (group.length === 1) {
      ...
      L.marker(bounds.getCenter(), {
        ...
        .on("click", () => {
          if (zoom < 17) {
            map.fitBounds(bounds, { padding: [55, 55], maxZoom: zoom + 3 });
            return;
          }
          const list = node("div", undefined, "cluster-list");
          for (const feature of group) {
            const button = node("button", feature.properties.name);
            button.onclick = () => {
              map.closePopup();
              loadPoiDetail(feature.properties.poi_id);
            };
            list.append(button);
          }
...
async function loadMapPois() {
  ...
  try {
    const data = await api("/api/v2/map-pois?" + params);
...
async function initializeMap() {
  if (!map) return;
  map.createPane("nationalPane");
  map.getPane("nationalPane").style.zIndex = "180";
  map.createPane("basemapPane");
  map.getPane("basemapPane").style.zIndex = "190";
  map.getPane("basemapPane").style.pointerEvents = "none";
  basemapRenderer = L.canvas({ pane: "basemapPane", padding: 0.5 });
  markerLayer = L.layerGroup().addTo(map);
  onlineTiles = L.tileLayer("https://tile.openstreetmap.de/{z}/{x}/{y}.png", {
    maxZoom: 18,
    attribution:
      '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> · Tiles: <a href="https://www.openstreetmap.de/">FOSSGIS</a>',
  });
  onlineTiles.on("tileload", () => {
    tilesLoaded++;
    if (offline) setOffline(false);
  });
  onlineTiles.on("loading", () => {
    tilesLoaded = 0;
  });
  onlineTiles.on("load", () => setOffline(!tilesLoaded));
  onlineTiles.on("tileerror", () => {
    if (!tilesLoaded && !offline) setOffline(true);
  });
  onlineTiles.addTo(map);
  setTimeout(() => {
    if (!tilesLoaded) setOffline(true);
  }, 6000);
  let timer;
  map.on("moveend", () => {
    clearTimeout(timer);
    timer = setTimeout(loadMapPois, 180);
  });
  ...
  try {
    ...
    nationalLayer = L.geoJSON(data, {
      ...
      onEachFeature: (feature, layer) =>
        layer.bindTooltip(node("span", feature.properties.name)),
    }).addTo(map);
    nationalBounds = nationalLayer.getBounds();
    map.fitBounds(nationalBounds, { padding: [24, 24], animate: false });
  } catch {
    nationalBounds = L.latLngBounds([
      [7.1, 102],
      [23.5, 117.9],
    ]);
  ...
  for (const button of document.querySelectorAll("[data-map-location]"))
    button.onclick = () => {
      $("locationFilter").value = button.dataset.mapLocation;
